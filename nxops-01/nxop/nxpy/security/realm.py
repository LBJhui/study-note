...
from nxpy.pmx.management import Manager, ManagementGateway, Managable
...
class Securable:
    ...
    def _check_realm(self, realm):
        if not self.realm or (
                not self.realm.auth_id and not self.realm.auth_token and not self.realm.roles and not self.realm.auth_level):
            return True
        elif realm:
            accessable = True
